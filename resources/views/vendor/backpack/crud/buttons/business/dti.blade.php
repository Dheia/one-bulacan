@if ($crud->hasAccess('dti'))
    @if($entry->getAttributeValue('dti') == 1)
       {{--  <li><a class="btn btn-sm btn-primary active"><i class="fas fa-check"></i> DTI</a></li> --}}
    @else
    	<a href="javascript:void(0)" onclick="dti(this)" data-name="{{ $entry->getAttributeValue('name') }}" data-id="{{ $entry->getKey() }}" data-route="{{ url($crud->route.'/dti/'.$entry->getKey()) }}" class="btn btn-sm btn-link" data-button-type="dti">
