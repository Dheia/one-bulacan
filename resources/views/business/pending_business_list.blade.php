  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            ...
            id="publishModalLabel"></h4>
            <button type="button" class="close" 
            data-dismiss="modal" 
            aria-label="Close">
            <span aria-hidden="true">&times;</span></button>
        </div>
          <form method="POST" action="{{ url($crud->route.'/publish') }}">
            @csrf
            {!! csrf_field() !!}
              <div class="modal-body">
                  <div class="form-group">
                      <input type="hidden" id="publish_business_id" name="publish_business_id" value="">
                      <label>Select publish lenght</label>
                      <select class="form-control" name="publish_lenght_of_time" id="publish_lenght_of_time">
    ...
    <script>
        $(function() {
            $('#publishModal').on("show.bs.modal", function (e) {
                $("#publish_business_id").val($(e.relatedTarget).data('id'));
                console.log($(e.relatedTarget).data('id'));
                $("#publishModalLabel").html($(e.relatedTarget).data('title'));
                $("#publish-title").html($(e.relatedTarget).data('title'));
            });
        });
    </script>
