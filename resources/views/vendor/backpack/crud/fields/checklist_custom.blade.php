...
@include('crud::fields.inc.wrapper_start')
    ...
    <div class="row">
        @foreach ($field['model']::orderBy('page_name')->get() as $key => $option)
          @if($category !== $option->page_name)
            <div class="col-md-12" style="padding-left: 30px;">
              <br>
              <label for="">{{ title_case(str_slug($option->page_name, ' ')) }}</label>
            </div>
            @php 
              $category = $option->page_name;
            @endphp
          @endif
            ...
            <div class="col-sm-3" style="padding-left: 40px;">
                <div class="checkbox">
                  <label class="font-weight-normal">
                    <input type="checkbox" value="{{ $key }}"> {{ $option->name }}
                  </label>
                </div>
            </div>
        @endforeach
