    ...
    @push('crud_fields_scripts')
        ...
        <script>
            function getCategories(){
                $.ajax({
                    url: "{{ route('business/api/get-categories') }}", 
                    data:{
                        parent_id: $('select[name="category_id"').find(":selected").val(),
                    },
                    success: function(result){
                        $('select[name="business_category[]"]').empty();
                        if(result == ''){
                            var option = '<option value">' + 'N/A' + '</option>';
                            $('select[name="business_category[]"]').append(option);
                        }
                        else{
                            $.each(result, function(index, value) {
                                // console.log('div' + index + ':', value);
                                $('select[name="business_category[]"]').prop('disabled', false);
