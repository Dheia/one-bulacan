@if ($crud->hasAccess('verified'))
    @if($entry->getAttributeValue('verified') == 1)
    	{{-- <li><a class="btn btn-sm btn-primary active"><i class="fas fa-check"></i> Verified</a></li> --}}
    @else
        <a href="javascript:void(0)" onclick="verify(this)" data-name="{{ $entry->getAttributeValue('name') }}" data-id="{{ $entry->getKey() }}" data-route="{{ url($crud->route.'/verify/'.$entry->getKey()) }}" class="btn btn-sm btn-link" data-button-type="verify">
