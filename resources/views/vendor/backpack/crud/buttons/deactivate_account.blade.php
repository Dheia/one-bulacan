@if ($crud->hasAccess('deactivate account'))
	@if($entry->credential)
		@if($entry->credential->active)
			<a href="javascript:void(0)" onclick="deactivateAccount(this)" data-id="{{ $entry->getKey() }}" data-route="{{ url($crud->route.'/deactivate-account/'.$entry->id) }}" class="btn btn-sm btn-default" data-button-type="deactivateAccount" title="Deactivate Account"><i class="fas fa-user-alt"></i> </a>
		@endif
	@endif
@endif
